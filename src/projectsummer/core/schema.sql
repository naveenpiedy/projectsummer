...
COMMENT ON COLUMN films.directors IS 'Directors as TMDB credits them. A list, NULL when unknown: filter with list_contains(directors, ''Name''), which needs the exact spelling. Names only, and two people can share one: to count or describe people, join film_credits and people by person_id instead.';
COMMENT ON COLUMN films.cast_members IS 'Up to ten leading cast members, in billing order. A list. (Not called cast, which is a SQL keyword.) Names only, and two people can share one: to count or describe people, join film_credits and people by person_id instead.';
COMMENT ON COLUMN films.writers IS 'Screenplay, writer, story, novel and dialogue credits. A list. Names only, and two people can share one: to count or describe people, join film_credits and people by person_id instead.';
COMMENT ON COLUMN films.composers IS 'Composers of the original music. A list. Names only, and two people can share one: to count or describe people, join film_credits and people by person_id instead.';
COMMENT ON COLUMN films.cinematographers IS 'Directors of photography. A list. Names only, and two people can share one: to count or describe people, join film_credits and people by person_id instead.';
COMMENT ON COLUMN films.editors IS 'Editors. A list. Names only, and two people can share one: to count or describe people, join film_credits and people by person_id instead.';
